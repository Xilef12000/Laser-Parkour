void laserMonitoring() {
  ...
  while (digitalRead(stop_pin) == 1) { //repeat while stop button is not pressed
    for (int i = diod_pin; i <= diod_pin + diods - 1; i++) { //controll the Lasers one by one
      //Serial.println(String(i) + ": " + String(analogRead(i)));
      if (analogRead(i) >= trigger_value) { //if actual light level higher then triggerlevel
        Serial.println("laserMonitoring: " + String(i) + " rose above " + String(trigger_value)); //print which laser got interrupted
        penalty_time = penalty_time + penalty; //add penalty time
        noTone(speaker_pin); //stop all tones
        tone(speaker_pin, laser_beep, beep_length); //play laser interruption tone
        while (analogRead(i) >= trigger_value) {} //wait for laser to be not interrupted
      }
    }
